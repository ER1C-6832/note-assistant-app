...
class UIPlugin(Plugin):
    ...
    def __init__(self, mode: Optional[str] = None) -> None:
        ...
        self._is_gui = False
        self._is_headless = self.mode == "headless"
        self.is_first = True
    ...
    def _create_view_manager(self):
        ...
        if self.mode == "headless":
            ...
            self.view_manager = None
            logger.info("Headless 模式，跳过 UI 初始化")
            return
    ...
    async def start(self) -> None:
        ...
        # Headless 模式不创建窗口/CLI 显示，但保留 UIPlugin 的事件处理能力。
        if self._is_headless:
            logger.info("UIPlugin headless 模式已启动（无 GUI/CLI 窗口）")
            return
