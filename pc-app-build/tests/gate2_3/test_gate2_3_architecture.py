...
def test_dependency_verifier_and_real_acceptance_runner_are_present() -> None:
    pyproject = _read(PC_BUILD_ROOT / "pyproject.toml")
    verifier = _read(PC_BUILD_ROOT.parent / "VERIFY_GATE2_3.ps1")
    real_runner = _read(PC_BUILD_ROOT.parent / "RUN_GATE2_3_REAL_WEBSOCKET_HELLO.ps1")

    assert '"websockets>=16.0,<17"' in pyproject
    assert "import websockets" in verifier
    assert "tests/gate2_3" in verifier
    assert "$LASTEXITCODE -ne 0" in verifier
    assert "exit 1" in verifier
    assert "verify_gate2_3_real_websocket.py" in real_runner
