"""Asynchronous note mutation boundary shared by UI and future MCP tools."""
...
class NoteCommandService:
    ...
    async def set_pinned(self, command: SetPinnedCommand) -> tuple[Note, ...]:
        return await run_repository_call(
            self._executor,
            "set_pinned",
            self._repository.set_pinned_many,
            command,
        )
    ...
    async def soft_delete(self, command: SoftDeleteCommand) -> int:
        return await run_repository_call(
            self._executor,
            "soft_delete",
            self._repository.soft_delete_many,
            command,
        )
    ...
    async def hard_delete(self, command: HardDeleteCommand) -> int:
        return await run_repository_call(
            self._executor,
            "hard_delete",
            self._repository.hard_delete_many,
            command,
        )
