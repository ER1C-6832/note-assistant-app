"""Run the canonical non-interactive Gate 1 through Gate 5.4 verifier."""
...
def _checks(
    *,
    ...
    include_gate5_real_manual: bool,
) -> list[Check]:
    ...
    checks = [
        ...
        Check("gate5_4_freeze", (python, "tools/verify_gate5_4_freeze.py")),
    ]
    ...
    if include_gate5_real:
        ...
        if include_gate5_real_manual:
            checks.append(
                Check(
                    "gate5_4_real_manual",
                    (
                        python,
                        "tools/verify_gate5_4_real_manual.py",
                        "--confirm-effects",
                    ),
                    interactive=True,
                )
            )
        elif include_gate5_real_ui_language:
            checks.append(
                Check(
...
def main() -> int:
    ...
    parser.add_argument(
        "--include-gate5-real-manual",
        action="store_true",
        help=(
            "launch the actual desktop UI and record user-authored natural-language "
            "commands for the frozen Gate 5.4 catalog; implies --include-gate5-real"
        ),
    )
    parser.add_argument(
    ...
    include_gate5_real = bool(
        args.include_gate5_real
        or args.include_gate5_real_ui_language
        or args.include_gate5_real_manual
    )
    expected = _checks(
        ...
        include_gate5_real_manual=args.include_gate5_real_manual,
    )
    results: list[dict[str, object]] = []
    for check in expected:
        print(f"[Gate 5.4] {check.name}", file=sys.stderr, flush=True)
    ...
    print(
        json.dumps(
            {
                "status": (
                    "gate5_4_cumulative_complete"
                    if passed
                    else ("gate5_4_cumulative_blocked" if blocked else "failed")
                ),
                "baseline_expectation": "single-process Gate 1.1 through Gate 5.4",
                "include_gate4_real": args.include_gate4_real,
                "include_gate5_real": include_gate5_real,
                "include_gate5_real_ui_language": (args.include_gate5_real_ui_language),
                "include_gate5_real_manual": args.include_gate5_real_manual,
