"""Run the canonical non-interactive Gate 1 through Gate 5.0 verifier."""
...
def main() -> int:
    ...
    for check in expected:
        print(f"[Gate 5.0] {check.name}", file=sys.stderr, flush=True)
    ...
    print(
        json.dumps(
            {
                "status": (
                    "gate5_0_cumulative_complete"
                    if passed
                    else ("gate5_0_cumulative_blocked" if blocked else "failed")
                ),
                "baseline_expectation": "single-process Gate 1.1 through Gate 5.0",
                "include_gate4_real": args.include_gate4_real,
                "include_gate5_real": args.include_gate5_real,
                "checks": results,
